...
def save_single_query():
    query = input("请输入问题（回车结束）：").strip()
    if not query:
        print("输入为空，未保存。")
        return
    data = [[query, 0]]  # 固定格式
    path = r"E:\IDADM\dataset\nq320k\dev.json"
...
def test(config):
    ...
    filtered_data = []
    for idx, (query, doc_id) in enumerate(data):
        if isinstance(doc_id, int) and doc_id <= 99739:
            filtered_data.append([query, doc_id])

    print(f"after filtered: {len(filtered_data)}")
    data = filtered_data
    ...
    for epoch in range(epochs):
        ...
        corpus_ids = [[0, *line] for line in json.load(open(f'{save_path}/{epoch}.pt.all.code'))]
        ...
        # ✅ 新增：构造前5条 query 的 Top-3 预测结果并保存为 JSON
        results = []
        for i in range(min(5, len(data))):
            query_text, true_docid = data[i]
            pred_docids = predictions[i][:3] if i < len(predictions) else []
            pred_docs = [corpus[docid] for docid in pred_docids if 0 <= docid < len(corpus)]
            true_doc = corpus[true_docid] if 0 <= true_docid < len(corpus) else ""
            results.append({
                "query": query_text,
                "true_docid": true_docid,
                "true_doc": true_doc,
                "predicted_docids": pred_docids,
                "predicted_docs": pred_docs
            })

        out_file = f"{save_path}/top3_pred.json"
        with open(out_file, "w", encoding="utf-8") as f:
            json.dump(results, f, indent=2, ensure_ascii=False)
        print(f"Top-3 预测结果已保存到 {out_file}")

        from eval import eval_all
        print('*'*100)
        print('Test', eval_all(predictions, [[x] for x in query_ids]))
        print('*'*100)        
...
def test_dr(config):
    ...
    for i in range(min(5, len(data))):
        ...
        print(f"   doc: {corpus[str(doc_id)]}...")  # 只打印前200字符

    # ✅ 构造一个小数据集用于模型前向
    small_data = data[:5]
    small_dataset = BiDataset(data=small_data, corpus=corpus, tokenizer=tokenizer, max_doc_len=128, max_q_len=32, ids=corpus_ids, aux_ids=aux_ids)
    small_loader = torch.utils.data.DataLoader(small_dataset, collate_fn=small_dataset.collate_fn, batch_size=5, shuffle=False, num_workers=0)
    ...
    with torch.no_grad():
        for batch in small_loader:
            batch = {k: v.cuda() for k, v in batch.items() if v is not None}
            output: QuantizeOutput = model(
                input_ids=batch['query'],
                attention_mask=batch['query'].ne(0),
                decoder_input_ids=batch['ids'],
                aux_ids=None,
                return_code=True,
                return_quantized_embedding=True,
            ...
            for i in range(len(batch['query'])):
                model_outputs.append({
                    "query": tokenizer.decode(batch['query'][i], skip_special_tokens=True),
                    "doc": tokenizer.decode(batch['doc'][i], skip_special_tokens=True),
                    "discrete_codes": output.discrete_codes[i].cpu().tolist(),
                    "continuous_embeds": output.continuous_embeds[i].cpu().tolist(),
                    "quantized_embeds": output.quantized_embeds[i].cpu().tolist() if output.quantized_embeds is not None else None,
                    "probability": output.probability[i].cpu().tolist()
                })

    # ✅ 保存到文件
    with open("top3_pred.json", "w", encoding="utf-8") as f:
        json.dump(model_outputs, f, ensure_ascii=False, indent=2)

    print("✅ 已保存前5条数据的模型输出到 top3_pred.json")
...
def main():
    ...
    config = {
        ...
        'epochs': 1,  
        'dev_data': './dataset/nq320k/dev.json',    
